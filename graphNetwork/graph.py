"""
Module for constructing and executing a multi-agent state graph for organizational analysis.

This module builds a multi-agent system using `langgraph`, where agents analyze specific aspects
of a given organizational task. The system concludes with a summary report generated by a
summary agent. The workflow integrates data from predefined sources, processes inputs, and
produces an actionable report.

Key Features:
    - Load data (questionnaires, best practices) from JSON files.
    - Build a state graph connecting agents and defining workflows.
    - Generate a visualization of the state graph in PNG format.
    - Execute the graph with user inputs and produce a final report.
...
Modules and Classes Used:
    ...
    - graphNetwork.nodes.agent_nodes: Contains the definitions of individual agent nodes.
    - graphNetwork.nodes.report_writer_node: Handles the generation of the final report.
...
Outputs:
    ...
    - `output.md`: The final report generated by the `Summary_Agent`.
